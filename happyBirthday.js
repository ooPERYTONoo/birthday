...
(hh = h / 2),
  (opts = {
    ...
    strings: ["HAPPY", "BIRTHDAY!", "to You", w_name],
    charSize: 30,
    charSpacing: 35,
    lineHeight: 40,
...
function anim() {
  ...
  if (done && !animationEnded) {
    animationEnded = true; // Prevent further animation
    console.log("Animation finished.");
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = "cakeCut.css";
    document.head.appendChild(link);
    const links = document.querySelectorAll('link[rel="stylesheet"]');
    links.forEach(link => {
      if (link.href.includes("birthdayOverlay.css")) {
        link.remove();
      }
    });

    nextAnimation();
    // Optional: You can trigger some message or UI here
  }
}
...
function nextAnimation() {
  let inner = `
  <div class="cake-card" id="initial-container" >
